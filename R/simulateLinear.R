                           ...
                           retries)
{
  ...
  PostFOCsFun <- function(price) {
    priceEffect <- MergerEffects::demandLinear(intercept=intercept,
                                                  price=price,
                                                  slope=slope,
                                                  eta=eta,
    ...
    for (i in 1:n) {
      for (j in 1:n) {
        demandEffect[i,j] <- eta*slope[i,j]*(price[i]-c1[i])*(intercept[i]+sum(slope[i,]*price))^(eta-1) # effect of p_j on q_i for all i,j
      }
    }
