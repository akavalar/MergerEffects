...
interceptLinLog <- function(slope,
                            p0,
                            q0,
                            R,
                            PoD,
                            ...
                            retries)
{
  ...
  demandEqnsFun <- function(intercept) {
    demandEqns <- demandLinLog(intercept=intercept,
                               price=p0,
                               slope=slope,
                               n=n) - q0
    return(demandEqns)
  ...
  if (diagnostics==0) {
    ...
    divJacobian <- numDeriv::jacobian(MergerEffects::demandLinLog,p0,intercept=intercept,
    ...
    checkDemand <- (abs(q0 - MergerEffects::demandLinLog(intercept=intercept,
                                                            price=p0,
                                                            slope=slope,
                                                            n=n)) < tol)
    
    # helper profit function
    profitLinLog <- function(price) {
      demand <- MergerEffects::demandLinLog(intercept=intercept,
                                               price=price,
                                               slope=slope,
                                               n=n)
    ...
    FOCsPreFun <- function(price) {
      FOCs <- diag(numDeriv::jacobian(profitLinLog,price))
      return(FOCs)
    }
