...
interceptLinear <- function(slope,
                            p0,
                            q0,
                            eta,
                            R,
                            ...
                            retries)
{
  ...
  demandEqnsFun <- function(intercept) {
    demandEqns <- c()
    for (i in 1:n) {
      demandEqns[i] <- (intercept[i] + sum(slope[i,]*p0)) - q0[i]^(1/eta)
    }
    return(demandEqns)
  }
  ...
  if (diagnostics==0) {
    ...
    divJacobian <- numDeriv::jacobian(MergerEffects::demandLinear,p0,intercept=intercept,
                                      slope=slope,eta=eta,n=n) # matrix of 1st numerical derivatives wrt p0 given intercept
    ...
    checkDemand <- (abs(q0 - MergerEffects::demandLinear(intercept=intercept,
                                                            price=p0,
                                                            slope=slope,
                                                            eta=eta,
                                                            n=n)) < tol)
    ...
    profitLinear <- function(price) {
      demand <- MergerEffects::demandLinear(intercept=intercept,
                                               price=price,
                                               slope=slope,
                                               eta=eta,
                                               n=n)
    ...
    FOCsPreFun <- function(price) {
      FOCs <- diag(numDeriv::jacobian(profitLinear,price))
      return(FOCs)
    }
