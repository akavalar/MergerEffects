                         ...
                         retries)
{
  ...
  PostFOCsFun <- function(price) {
    priceEffect <- MergerEffects::demandAIDS(intercept=intercept,
                                                price=price,
                                                slope=slope,
    ...
    for (i in 1:n) {
      for (j in 1:n) {
        demandEffect[i,j] <- ifelse(i==j, (price[i]-c1[i])*price[i]^(-2)*(slope[i,j]-(intercept[i] + sum(slope[i,]*log(price)))),
                                    (price[i]-c1[i])*(1/price[i]*1/price[j])*slope[i,j]) # effect of p_j on q_i for all i,j
      }
    }
